...
const DataParsing: React.FC = () => {
  ...
  const renderProcessingTypeIcon = (type: ProcessingType) => {
    switch (type) {
      case 'structuring': return <Layers className="h-4 w-4" />;
      case 'cleaning': return <PenTool className="h-4 w-4" />;
      case 'ner': return <FileSearch className="h-4 w-4" />;
      case 'topics': return <Database className="h-4 w-4" />;
      case 'summarization': return <FileText className="h-4 w-4" />;
      case 'sentiment': return <SmilePlus className="h-4 w-4" />;
  ...
  const renderProcessingTypeLabel = (type: ProcessingType) => {
    switch (type) {
      case 'structuring': return 'Auto-Structuring';
      case 'cleaning': return 'Data Cleaning';
      case 'ner': return 'Named Entity Recognition';
      case 'topics': return 'Topic Extraction';
  ...
  return (
    <div className="container mx-auto px-4 py-6 max-w-7xl">
      <div className="space-y-2 mb-8">
        ...
        >
          Upload data files, analyze structure, process with AI, and generate additional data points
        </motion.p>
      </div>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <FileUploader 
                        ...
                        accept=".csv,.json,.txt,.pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.jpg,.jpeg,.png"
                        maxSize={10}
